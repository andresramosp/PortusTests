...
export default {
  ...
  methods: {
    getTableData() {
      var dt = this;
      ApiService.post(
        "RTData/" + this.marker.id + "?locale=" + this.$getLocale(), this.parameters.map(p => p.id))
        .then(result => {
          if (result.data.length > 0) {
            this.dataSource = result.data.map((row) => this.formatRow(row) )
            this.columnsNames = this.generateColumnsNames(result.data[0]);
          }
        });
    },
